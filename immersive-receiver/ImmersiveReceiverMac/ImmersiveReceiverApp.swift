...
class AppState: ObservableObject {
    ...
    init() {
        ...
        let savedName = UserDefaults.standard.string(forKey: "receiverName") ?? Self.defaultReceiverName()
        ...
        if let renderer = VideoRenderer.create() {
            self.videoRenderer = renderer
            print("[AppState] Metal renderer initialized successfully")
        } else {
            self.videoRenderer = nil
            print("[AppState] WARNING: Metal renderer failed to initialize - video will not display")
        }
