//! Poop Rain Effect
//!
//! Rains poop emojis on video, using edge detection to make them land on shapes.
...
/// Poop Rain effect definition
pub struct PoopRainDefinition;

impl EffectDefinition for PoopRainDefinition {
    ...
    fn default_parameters(&self) -> Vec<Parameter> {
        vec![
            ...
            Parameter::new(ParameterMeta::float("particle_size", "Emoji Size", 1.0, 0.5, 2.0)),
    ...
    fn create_gpu_runtime(
        ...
        Some(Box::new(PoopRainRuntime::new(device, queue, output_format)))
    }
...
/// GPU runtime for Poop Rain effect
pub struct PoopRainRuntime {
    // Edge detection pass
    ...
    emoji_texture_view: wgpu::TextureView,
...
impl PoopRainRuntime {
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue, output_format: wgpu::TextureFormat) -> Self {
        ...
        let edge_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Poop Rain Edge Shader"),
        ...
        let edge_bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Poop Rain Edge Bind Group Layout"),
            entries: &[
        ...
        let edge_pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("Poop Rain Edge Pipeline Layout"),
            bind_group_layouts: &[&edge_bind_group_layout],
            push_constant_ranges: &[],
        });

        let edge_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("Poop Rain Edge Pipeline"),
        ...
        let copy_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Poop Rain Copy Shader"),
        ...
        let copy_bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Poop Rain Copy Bind Group Layout"),
            entries: &[
                wgpu::BindGroupLayoutEntry {
                    binding: 0,
                    visibility: wgpu::ShaderStages::FRAGMENT,
        ...
        let particle_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Poop Rain Particle Shader"),
        ...
        let particle_bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Poop Rain Particle Bind Group Layout"),
            entries: &[
                wgpu::BindGroupLayoutEntry {
                    binding: 0,
                    visibility: wgpu::ShaderStages::FRAGMENT,
        ...
        let particle_pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("Poop Rain Particle Pipeline Layout"),
        ...
        let particle_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("Poop Rain Particle Pipeline"),
        ...
        let quad_vertex_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Poop Rain Quad Vertex Buffer"),
        ...
        let instance_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Poop Rain Instance Buffer"),
        ...
        let (emoji_texture, emoji_texture_view) = Self::create_emoji_texture(device, queue);
    ...
    fn create_emoji_texture(device: &wgpu::Device, queue: &wgpu::Queue) -> (wgpu::Texture, wgpu::TextureView) {
        // Load embedded PNG of poop emoji
        let png_data = include_bytes!("poop_emoji.png");

        let img = image::load_from_memory(png_data)
            .expect("Failed to load embedded poop emoji")
            .to_rgba8();
    ...
    fn ensure_edge_texture(&mut self, device: &wgpu::Device, width: u32, height: u32) {
        if self.edge_width != width || self.edge_height != height {
            let texture = device.create_texture(&wgpu::TextureDescriptor {
                label: Some("Poop Rain Edge Texture"),
                size: wgpu::Extent3d {
                    width,
                    height,
                    depth_or_array_layers: 1,
            ...
            let collision_texture = device.create_texture(&wgpu::TextureDescriptor {
                label: Some("Poop Rain Collision Texture"),
                size: wgpu::Extent3d {
                    width: coll_width,
                    height: coll_height,
                    depth_or_array_layers: 1,
            ...
            self.collision_staging_buffer = Some(device.create_buffer(&wgpu::BufferDescriptor {
                label: Some("Poop Rain Collision Staging Buffer"),
                size: buffer_size,
    ...
    fn simulate_particles(&mut self, params: &EffectParams) {
        ...
        let density = params.params[0];
        let landing_mode = params.params[1] as u32;
        let gravity = params.params[2] * 0.3;
        let edge_threshold = params.params[3];
        let size_scale = params.params[4];
        let bounce_dampening = params.params[5];
        let fade_duration = params.params[6];
...
impl GpuEffectRuntime for PoopRainRuntime {
    fn process(
        &mut self,
        ...
        // Pass 1a: Edge detection (full res for display)
        {
            let edge_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("Poop Rain Edge Bind Group"),
            ...
            let mut edge_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("Poop Rain Edge Pass"),
        ...
        {
            let edge_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("Poop Rain Collision Edge Bind Group"),
                layout: &self.edge_bind_group_layout,
                entries: &[
                    wgpu::BindGroupEntry {
                        binding: 0,
            ...
            let mut coll_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("Poop Rain Collision Edge Pass"),
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    view: collision_view,
                    resolve_target: None,
                    ops: wgpu::Operations {
        ...
        // Pass 2: Copy input to output
        {
            let copy_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("Poop Rain Copy Bind Group"),
            ...
            let mut copy_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("Poop Rain Copy Pass"),
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    view: output,
                    resolve_target: None,
                    ops: wgpu::Operations {
        ...
        if !instances.is_empty() {
            let particle_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("Poop Rain Particle Bind Group"),
                layout: &self.particle_bind_group_layout,
                entries: &[
                    wgpu::BindGroupEntry {
                        binding: 0,
                        resource: wgpu::BindingResource::TextureView(&self.emoji_texture_view),
                    },
                    wgpu::BindGroupEntry {
                        binding: 1,
                        resource: wgpu::BindingResource::Sampler(&self.sampler),
            ...
            let mut particle_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("Poop Rain Particle Pass"),
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    view: output,
                    resolve_target: None,
                    ops: wgpu::Operations {
