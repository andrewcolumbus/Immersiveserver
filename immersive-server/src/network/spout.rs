...
//!
//! This module provides GPU texture sharing via Spout, allowing the environment
...
///
/// Shares the environment texture via DirectX shared textures.
/// This provides low-latency texture sharing with other applications.
...
impl SpoutSharer {
    ...
    pub fn set_dimensions(&mut self, width: u32, height: u32) {
        if self.width != width || self.height != height {
            self.width = width;
            self.height = height;
            // Resize pixel buffer (BGRA = 4 bytes per pixel)
    ...
    ///
    /// The pixels should be in BGRA format, matching the environment texture.
    pub fn publish_pixels(&mut self, pixels: &[u8]) -> Result<(), String> {
        if let Some(ref spout) = self.spout {
            ...
            // Send via Spout (BGRA format, not inverted since we handle that)
    ...
    ///
    /// This can be used for GPU readback to avoid allocations.
    pub fn pixel_buffer_mut(&mut self) -> &mut Vec<u8> {
        &mut self.pixel_buffer
    }

    /// Get the expected pixel buffer size.
...
impl TextureSharer for SpoutSharer {
    fn start(&mut self, name: &str) -> Result<(), String> {
        ...
        // Resize pixel buffer for the current dimensions
        self.pixel_buffer
    ...
    fn publish_frame(&mut self) -> Result<(), String> {
        // This is called from the trait interface without pixel data.
        // The actual publishing should be done via publish_pixels() after GPU readback.
        if !self.active {
            return Err("Spout sender not started".to_string());
        }

        // The render loop should call publish_pixels() with the readback data
        Ok(())
    }

    fn has_receivers(&self) -> bool {
        // Spout doesn't provide a direct way to check for receivers
        // Return true if sender is active
        self.active
    }
