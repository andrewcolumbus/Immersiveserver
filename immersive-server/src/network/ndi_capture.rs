//! NDI (Network Device Interface) frame capture from GPU.
//!
//! This module handles capturing the compositor's environment texture
//! and preparing it for transmission over NDI.
//!
//! # Architecture
//!
...
/// Handles GPU texture capture for NDI streaming.
///
/// Uses triple-buffered async capture to avoid blocking the render loop.
...
pub struct NdiCapture {
    ...
    frame_tx: Option<mpsc::SyncSender<NdiCapturedFrame>>,
...
impl NdiCapture {
        ...
        let buffers: Vec<StagingBuffer> = (0..NUM_STAGING_BUFFERS)
            .map(|i| {
                let buffer = device.create_buffer(&wgpu::BufferDescriptor {
                    label: Some(&format!("NDI Staging Buffer {}", i)),
                    size: buffer_size,
    ...
    ///
    /// The thread receives captured frames and sends them via the NdiSender.
    pub fn start_sender_thread(&mut self, mut sender: NdiSender) {
            ...
            .spawn(move || {
                ...
                loop {
                    ...
                    match frame_rx.recv_timeout(Duration::from_millis(100)) {
                        Ok(frame) => {
                            // Send frame via NDI (async - non-blocking)
                            let result = sender.send_frame_async(
                                frame.width,
                                frame.height,
                                &frame.data,
                            );
                            ...
                            // Keep frame alive until next send (async send holds reference)
                            _prev_frame = Some(frame);
                        }
