...
//!
//! This module provides GPU texture sharing via Syphon, allowing the output
...
///
/// Shares the output texture directly on the GPU via Syphon's Metal support.
    ...
    pub unsafe fn publish_wgpu_texture(
        ...
        let metal_texture: Option<metal::Texture> =
            texture.as_hal::<wgpu_hal::api::Metal, _, _>(|hal_texture| {
                hal_texture.map(|tex| tex.raw_handle().to_owned())
            });
        ...
        let linear_view = metal_texture.new_texture_view(metal::MTLPixelFormat::BGRA8Unorm);
